 ...
 */
int API::getIntegerResponse(const std::string& commandUsed) {
    std::string response = getResponse(commandUsed);
    try {
 ...
 */
bool API::getBooleanResponse(const std::string& commandUsed) {
    std::string response = getResponse(commandUsed);
 ...
 */
bool API::getAck(const std::string& commandUsed) {
    std::string response = getResponse(commandUsed);
    return response == "ack";
}
...
void API::moveForward() {
    ...
    if (ack) {
        ...
        std::cerr << mouseLocal->localMazeToString();
...
void API::ackReset() {
    ...
    if (!ack) {
        std::cerr << "Failed to acknowledge reset." << std::endl;
